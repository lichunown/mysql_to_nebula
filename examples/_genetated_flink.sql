CREATE TABLE IF NOT EXISTS t_user_info_mysql( 
	...
	`sex` INTEGER NULL COMMENT '性别 0男；1女', 
	`birthday` DATE NULL COMMENT '生日', 
	`in_Beijing` INTEGER NULL COMMENT '是否在北京 1是 0否', 
	`status_type` INTEGER NULL COMMENT '信息状态 0待填报；1待确认(管理员)；2待确认(家人)；3已确认；-1无证件号用户', 
	`create_user` VARCHAR(32) NOT NULL COMMENT '创建用户id', 
	`create_time` DATETIME NOT NULL COMMENT '创建时间', 
	`create_app` VARCHAR(32) NULL COMMENT '创建appId', 
	`modify_user` VARCHAR(32) NOT NULL COMMENT '更新用户id', 
	`modify_time` DATETIME NOT NULL COMMENT '修改时间', 
	`modify_app` VARCHAR(32) NULL COMMENT '更新appId', 
...
CREATE TABLE IF NOT EXISTS t_region_mysql( 
	`id` INTEGER NOT NULL COMMENT '主键', 
	`region_id` VARCHAR(64) NULL COMMENT '', 
	`parent_region_id` VARCHAR(64) NULL COMMENT '', 
	`region_code` VARCHAR(64) NULL COMMENT '地址编码', 
	`parent_region_code` VARCHAR(64) NULL COMMENT '父地址编码', 
	`region_name` VARCHAR(320) NULL COMMENT '地址名称', 
	`region_type` TINYINT NULL COMMENT '地址类型，1-省 2-市 3-区县 4-乡镇/街道 5-社区/村屯  6-住宅小区 7-楼栋  8-单元  9-住户', 
	`full_address` VARCHAR(2000) NULL COMMENT '地址全层次名称', 
	`leaf` TINYINT NULL COMMENT '是否叶子节点 0否 1是', 
	`create_time` TIMESTAMP NOT NULL COMMENT '创建时间', 
	`modify_time` TIMESTAMP NOT NULL COMMENT '修改时间', 
	`del_stat` TINYINT NULL COMMENT '0：正常 1：删除', 
	`version_no` INTEGER NULL COMMENT '', 
	PRIMARY KEY (`id`) NOT ENFORCED 
 ) WITH ( 
	'connector' = 'mysql-cdc', 
	'hostname' = '172.16.1.7', 
...
CREATE TABLE IF NOT EXISTS t_user_region_mysql( 
	`id` INTEGER NOT NULL COMMENT '主键', 
	`user_region_id` VARCHAR(64) NULL COMMENT '唯一id', 
	`user_id` VARCHAR(32) NOT NULL COMMENT '用户ID', 
	`region_id` VARCHAR(32) NULL COMMENT '地址编码', 
	`region_used_type` INTEGER NULL COMMENT '常驻地址类型 1家庭居住; 2工作常驻; 3外派出差; 4学校进修; 5度假疗养; 6隔离管控；7其他', 
	`start_time` DATE NULL COMMENT '驻留开始时间', 
	`end_time` DATE NULL COMMENT '驻留结束时间', 
	`duration_type` INTEGER NULL COMMENT '驻留周期 1持续驻留；2工作日驻留；3周末驻留；4每周1-3天；5每周4-6天', 
	`duration_cost_type` INTEGER NULL COMMENT '驻留费用承担方式 1自有购买；2个人租赁；3单位承担；4政府安置；5安赠继承', 
	`status_type` INTEGER NULL COMMENT '信息状态 0待填写；1待确认(管理员)；2待确认(家人)；3已确认', 
	`region_context` STRING NULL COMMENT '地址描述', 
	`building` VARCHAR(64) NULL COMMENT '楼/栋', 
	`unit` VARCHAR(32) NULL COMMENT '单元', 
	`room` VARCHAR(32) NULL COMMENT '门牌号', 
	`create_user` VARCHAR(32) NULL COMMENT '创建用户id', 
	`create_app` VARCHAR(32) NULL COMMENT '创建app', 
	`create_time` DATETIME NOT NULL COMMENT '创建时间', 
	`modify_user` VARCHAR(32) NULL COMMENT '更新用户id', 
	`modify_app` VARCHAR(32) NULL COMMENT '更新app', 
	`modify_time` DATETIME NOT NULL COMMENT '修改时间', 
	`del_stat` TINYINT NOT NULL COMMENT '0：正常 1：删除', 
	`version_no` INTEGER NOT NULL COMMENT '版本号', 
	`residence_free_text` VARCHAR(256) NULL COMMENT '', 
	PRIMARY KEY (`id`) NOT ENFORCED 
 ) WITH ( 
	'connector' = 'mysql-cdc', 
	'hostname' = '172.16.1.7', 
