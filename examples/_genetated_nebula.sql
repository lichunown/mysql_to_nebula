CREATE SPACE jmt2 (partition_num=120, replica_factor=1, vid_type=FIXED_STRING(64));
...
CREATE TAG INDEX IF NOT EXISTS `idx_region_id` ON region(`region_id`);
...
COMMENT='常驻地址表'; 
CREATE EDGE INDEX IF NOT EXISTS `user_region_id_UNIQUE` ON user_region_relation(`user_region_id`);
CREATE EDGE INDEX IF NOT EXISTS `idx_user_region_id` ON user_region_relation(`region_id`, `user_id`);
...
CREATE EDGE INDEX IF NOT EXISTS `t_region_address_area_code_IDX` ON region_region_relation(`region_code`, `del_stat`);
CREATE EDGE INDEX IF NOT EXISTS `t_region_address_parent_area_code_IDX` ON region_region_relation(`parent_region_code`);
CREATE EDGE INDEX IF NOT EXISTS `idx_modify_time` ON region_region_relation(`modify_time`);
CREATE EDGE INDEX IF NOT EXISTS `full_address` ON region_region_relation(`full_address`);
CREATE EDGE INDEX IF NOT EXISTS `region_type` ON region_region_relation(`region_type`);
CREATE EDGE INDEX IF NOT EXISTS `parent_region_address_id` ON region_region_relation(`parent_region_id`, `del_stat`);
CREATE EDGE INDEX IF NOT EXISTS `idx_region_id` ON region_region_relation(`region_id`);
...
CREATE EDGE IF NOT EXISTS user_user_relation(`id` INT32 NOT NULL COMMENT '主键', \
	...
	`version_no` INT32 NOT NULL COMMENT '版本号') \
COMMENT='用户关系表'; 
CREATE EDGE INDEX IF NOT EXISTS `idx_relation_id` ON user_user_relation(`relation_id`);